 ...
 *
 * Answers live behind {@link revealAnswer} rather than inside {@link InklingPuzzle}
 * so a client can lay out the board -- and re-render it mid-session -- without ever
 * holding the solution it has not earned yet.
 */
import { INKLING_WORDS, type InklingWordLength } from '../data/inklingWords';
...
export const INKLING_ROUNDS = 5;
export const INKLING_LENGTHS = [4, 5, 5, 6, 6] as const;
export const INKLING_GENERATOR_VERSION = 1;
/** Day zero of the schedule. The shuffle is seeded from this, never from the date. */
export const INKLING_EPOCH = '2026-01-01';

export type LetterMark = 'correct' | 'present' | 'absent';

export interface InklingRound {
  index: number;
  length: number;
...
export interface InklingPuzzle {
  puzzleDate: string;
...
const toUtcMs = (puzzleDate: string): number => {
  if (!DATE_PATTERN.test(puzzleDate)) {
    throw new Error(`Invalid Inkling puzzle date: ${puzzleDate}`);
  }
...
const EPOCH_MS = toUtcMs(INKLING_EPOCH);

/** Whole days from {@link INKLING_EPOCH}; negative for dates before it. */
...
const isWordLength = (length: number): length is InklingWordLength =>
  length === 4 || length === 5 || length === 6;

const wordAt = (length: InklingWordLength, index: number): string => {
  const { words } = INKLING_WORDS[length];
...
const indexOfWord = (length: InklingWordLength, word: string): number => {
  const { count } = INKLING_WORDS[length];
  let low = 0;
  let high = count - 1;
  while (low <= high) {
...
const decodeBase64 = (input: string): Uint8Array => {
  ...
  for (let i = 0; i < input.length; i += 1) {
    ...
    if (value < 0) throw new Error('Malformed Inkling answer mask');
...
const maskCache = new Map<InklingWordLength, Uint8Array>();

const answerMaskFor = (length: InklingWordLength): Uint8Array => {
  const cached = maskCache.get(length);
  if (cached) return cached;
  const decoded = decodeBase64(INKLING_WORDS[length].answerMask);
  maskCache.set(length, decoded);
  return decoded;
};

/** Bits are most-significant-first, so word `i` is bit `0b1000_0000 >> (i & 7)` of byte `i >> 3`. */
const isAnswerEligible = (length: InklingWordLength, index: number): boolean => {
...
const DRAWS_PER_DAY: Record<number, number> = INKLING_LENGTHS.reduce<Record<number, number>>(
...
const ROUND_PLAN = INKLING_LENGTHS.map((length, index) => ({
  index,
  length,
  ordinal: INKLING_LENGTHS.slice(0, index).filter((other) => other === length).length,
}));

/** Lengths drawn more than once a day, ascending -- the pairs whose order the date seed decides. */
const PAIRED_LENGTHS = [...new Set(INKLING_LENGTHS)]
  .filter((length) => (DRAWS_PER_DAY[length] ?? 0) > 1)
  .sort((a, b) => a - b);

const permutationCache = new Map<InklingWordLength, Int32Array>();

const permutationFor = (length: InklingWordLength): Int32Array => {
  const cached = permutationCache.get(length);
  if (cached) return cached;

  const { count, answerCount } = INKLING_WORDS[length];
  const pool = new Int32Array(answerCount);
  let written = 0;
  for (let i = 0; i < count; i += 1) {
    if (isAnswerEligible(length, i)) {
      if (written >= answerCount) throw new Error(`Inkling mask over-reports length ${length}`);
      pool[written] = i;
      written += 1;
    }
  }
  if (written !== answerCount) {
    throw new Error(`Inkling answer mask for length ${length} sets ${written} bits, expected ${answerCount}`);
  }

  // Seeded from the epoch, never the date: one fixed order for all time.
  const rng = rngFromSeed(`${INKLING_EPOCH}:len${length}:v${INKLING_GENERATOR_VERSION}`);
...
const orderingFor = (puzzleDate: string): Record<number, boolean> => {
  ...
  if (cached) return cached;
  const rng = rngFromSeed(`${puzzleDate}:v${INKLING_GENERATOR_VERSION}`);
...
export const generateInkling = (puzzleDate: string): InklingPuzzle => {
  toUtcMs(puzzleDate);
  return {
    puzzleDate,
    generatorVersion: INKLING_GENERATOR_VERSION,
...
export const revealAnswer = (puzzleDate: string, roundIndex: number): string => {
  ...
  if (!plan || !Number.isInteger(roundIndex)) {
    throw new Error(`Inkling round index out of range: ${roundIndex}`);
  }
  const { length, ordinal } = plan;
  if (!isWordLength(length)) throw new Error(`Unsupported Inkling word length: ${length}`);
...
//   - `inklingScore` is the 0-100 number a player sees, and it is where guess
//     efficiency lives.
// ---------------------------------------------------------------------------
...
const roundPoints = ({ length, solved, guessesUsed }: InklingRoundOutcome): number => {
  if (!solved) return 0;
...
export const inklingScore = (outcomes: InklingRoundOutcome[]): number => {
  const total = outcomes.reduce((sum, outcome) => sum + roundPoints(outcome), 0);
  return Math.max(0, Math.min(100, total));
};

export const inklingSolvedCount = (outcomes: InklingRoundOutcome[]): number =>
 ...
 */
export const toInklingMetadata = (
  puzzle: InklingPuzzle,
  outcomes: InklingRoundOutcome[],
  durationSeconds: number,
): BrainTrainingMetadata => ({
  game: 'inkling',
  correct: inklingSolvedCount(outcomes),
  total: INKLING_ROUNDS,
  durationSeconds,
...
const asInklingEvent = (event: HealthEvent): HealthEvent<BrainTrainingMetadata> | null => {
  if (event.type !== 'BRAIN_TRAINING') return null;
  const metadata = event.metadata as BrainTrainingMetadata | null | undefined;
  if (!metadata || metadata.game !== 'inkling') return null;
  return event as HealthEvent<BrainTrainingMetadata>;
};

export const findInklingEventForDate = (
  events: HealthEvent[],
  puzzleDate: string,
): HealthEvent<BrainTrainingMetadata> | null => {
  for (const event of events) {
    const inkling = asInklingEvent(event);
    if (inkling && inkling.metadata.puzzleDate === puzzleDate) return inkling;
  }
  return null;
};
 ...
 */
export const inklingStreak = (events: HealthEvent[], today: Date = new Date()): StreakSummary => {
  const inklingEvents = events.filter((event) => asInklingEvent(event) !== null);
  return calculateStreaks(inklingEvents, today, (event) => {
